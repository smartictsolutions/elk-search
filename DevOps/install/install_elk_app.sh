...
if ! command -v docker &> /dev/null || ! command -v docker-compose &> /dev/null || ! command -v curl &> /dev/null || ! command -v yq &> /dev/null
then
  # yq için bkz. https://mikefarah.gitbook.io/yq/
  echo "docker, docker-compose, curl veya yq kurulu değil!"
  echo "yq             -> sudo apt update && sudo apt install snapd && sudo snap install yq"
...
echo "$(tput setaf 2)install_elk_app.sh tamamlandı.$(tput setaf 7)"
exit 0
